...
print(fp, tp)
if (tp < 30):
    os.system('rm model%d'%(modeli, index))
    exit(0)
run = 1
if run:
    ...
    joblib.dump(logreg, 'logreg%d.pkl'%(modeli, index))
else:
    logreg = joblib.load('logreg%d.pkl'%(modeli, index))
