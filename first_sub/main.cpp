...
//Compares the z coordinate value of two hits
inline bool z_cmp(const int a, const int&b) {
  return hits[a].z < hits[b].z;
}

//Compares the radius value of two hits
inline bool r_cmp(const int&a, const int&b) {
  return hits[a].x*hits[a].x+hits[a].y*hits[a].y < hits[b].x*hits[b].x+hits[b].y*hits[b].y;
}

//Something to do with detectors. Most likely some kind of map between layer ids and .... Find out what metai is!
...
double extendTripleLine(vector<triple>&triples, int ai, int bi, point&a, point&b, int li, PolarModule&mod, int rev = 0) {
  point d, dp, xp, bap;// Where are these declared?
...
int main(int argc, char**argv) {
  ...
  //}
  char file[1000];
  sprintf(file, "%s/adjacency", argv[2]);
  readHits();
  //storeAdjacent();
  loadAdjacent(file);
  //truthAdjacent();
  char file_detectors[1000];
  sprintf(file_detectors, "%s/detectors.csv", argv[2]);
  readDetectors(file_detectors);
  readCells();
  initHitDir();
  ...
  while (1) {//Actually only running a single iteration
    ...
    if (step <= 2) {
      {
      	vector<triple> straight_triples = findTriples(pairs, mod, 0);
    ...
    */
    ...
    //investigateAssignment(paths);
    //vector<int> to_zero (50000,0);
    map<int, int> map_assignment;
    for (int i = 1; i < hits.size(); i++){
      //cout << "Track id for hit id " << i << " is " << assignment[i] << endl;
      if (assignment[i]) {
        map_assignment[i] = assignment[i];
        //to_zero[assignment[i]]++;
      }
    }
    /*int j = 0;
    for (pair<int,int> i:map_assignment){
      if (to_zero[map_assignment[i.first]] < 8){
          j++;
          map_assignment[i.first] = 0;
      }
    }*/
    cout << "Number of hits with track having less than 8 hits" << j << endl;
    if (score <= 6) {
    ...
    char file_write[1000];
    sprintf(file_write, "%s/submission%d.csv", argv[2], filenum);
    writeSubmission(map_assignment, file_write);
