...
map<long long, vector<int> > truth_tracks; //truth hit ids in each track - creates a map from truth track to a vector containing truth hit ids
...
//reads truth.csv file and stores relevant
...
void readHits() {
  ...
  sprintf(file, "%s/%d-hits.csv", base_path, filenum);
...
//Assigns values to above created map for integers to Detectors
void readDetectors(char *base) {
  FILE*fp = fopen(base, "r");
  if (!fp) {
...
//Assigns values to above created map of <int, int> to hit_cells
void readCells() {
  char file[1000];
  //if (filenum >= 1000)
  ...
  sprintf(file, "%s/%d-cells.csv", base_path, filenum);
  FILE *fp = fopen(file, "r");
  if (!fp) {
